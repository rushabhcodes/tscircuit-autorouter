...
import { JumperTracePathSolver } from "./JumperTracePathSolver"
...
export interface FixedJumperAutorouterPipelineSolverOptions
  extends CapacityMeshSolverOptions {
  /**
   * A scaling factor applied to the default jumper spacing constant.
   * The resulting spacing applied to paired vias is
   * `JUMPER_DISTANCE / jumperDistance`.
   */
  jumperDistance?: number
}
...
type PipelineStep<T extends new (...args: any[]) => BaseSolver> = {
  ...
  getConstructorParams: (
    instance: FixedJumperAutorouterPipelineSolver,
  ) => ConstructorParameters<T>
  onSolved?: (instance: FixedJumperAutorouterPipelineSolver) => void
}
...
>(
  solverName: keyof FixedJumperAutorouterPipelineSolver,
  solverClass: T,
  getConstructorParams: (instance: FixedJumperAutorouterPipelineSolver) => P,
  opts: {
    onSolved?: (instance: FixedJumperAutorouterPipelineSolver) => void
  } = {},
): PipelineStep<T> {
...
export class FixedJumperAutorouterPipelineSolver extends BaseSolver {
  ...
  jumperTracePathSolver?: JumperTracePathSolver
  ...
  static readonly JUMPER_DISTANCE = 1.63
  private readonly jumperDistanceParameter: number
  private readonly targetJumperSpacing: number
  ...
  pipelineDef = [
    ...
    definePipelineStep(
      "jumperTracePathSolver",
      JumperTracePathSolver,
      (cms) => [
        {
          hdRoutes: cms._getPreJumperHdRoutes(),
          targetSpacing: cms.targetJumperSpacing,
        },
      ],
    ),
  ]
  ...
  constructor(
    ...
    public opts: FixedJumperAutorouterPipelineSolverOptions = {},
  ) {
    super()
    this.MAX_ITERATIONS = 100e6

    const invalidConnectionPoint = srj.connections
      .flatMap((connection) => connection.pointsToConnect)
      .find((point) => mapLayerNameToZ(point.layer, srj.layerCount) !== 0)

    if (invalidConnectionPoint) {
      throw new Error(
        "FixedJumperAutorouterPipelineSolver only supports pointsToConnect on the top layer (z=0).",
      )
    }

    this.jumperDistanceParameter = opts.jumperDistance ?? 1
    if (this.jumperDistanceParameter <= 0) {
      throw new Error("jumperDistance must be greater than 0")
    }
    this.targetJumperSpacing =
      FixedJumperAutorouterPipelineSolver.JUMPER_DISTANCE /
      this.jumperDistanceParameter
  ...
  _getPreJumperHdRoutes(): HighDensityRoute[] {
    return (
  ...
  _getOutputHdRoutes(): HighDensityRoute[] {
    return (
      this.jumperTracePathSolver?.getJumperRoutes() ??
      this._getPreJumperHdRoutes()
    )
  }
